...
import { Box, VStack, HStack, Text, Heading, Container, Grid, GridItem } from '@chakra-ui/react';
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedActivities) {
      ...
      setActivities(parsed.map((act: any) => ({
        ...act,
        timestamp: new Date(act.timestamp)
      })));
      
      // Calculate stats
      const totalStudyTime = parsed.reduce((acc: number, act: StudyActivity) => acc + act.duration, 0);
      const uniqueSubjects = new Set(parsed.map((act: StudyActivity) => act.subject)).size;
  ...
  const handleStartSession = (sessionData: { subject: string; notes: string; duration: number }) => {
    ...
    // Update stats
    const totalStudyTime = updatedActivities.reduce((acc, act) => acc + act.duration, 0);
    const uniqueSubjects = new Set(updatedActivities.map(act => act.subject)).size;
