...
const StudyPlanGenerator: React.FC<StudyPlanGeneratorProps> = ({
  ...
  const generateStudyPlan = async () => {
    if (units.length === 0) {
      toast({
        ...
        description:
          "Please add at least one study unit before generating a plan",
        status: "warning",
        duration: 3000,
        isClosable: true,
      });
  ...
  const generatePlanLogic = (): GeneratedPlan => {
    ...
    const totalAvailableHours =
      availability.dailyHours * availability.weeklyDays.length;
    const totalWeightScore = units.reduce(
      (sum, unit) => sum + unit.weightScore,
      0,
    );
    ...
    const unitAllocations = units.map((unit) => ({
      ...unit,
      allocatedHours:
        (unit.weightScore / totalWeightScore) * totalAvailableHours,
    }));

    // Check for potential issues
    if (totalAvailableHours < 10) {
      warnings.push(
        "Low total study hours - consider increasing daily availability",
      );
    }
    ...
    const upcomingExams = units.filter(
      (u) =>
        u.type === "exam" &&
        u.assessmentDate &&
        new Date(u.assessmentDate).getTime() - Date.now() <
          14 * 24 * 60 * 60 * 1000,
    );

    if (upcomingExams.length > 0) {
      warnings.push(
        `${upcomingExams.length} exams in next 2 weeks - increased focus recommended`,
      );
    }
    ...
    availability.weeklyDays.forEach((day, dayIndex) => {
      let dayHoursUsed = 0;
      const dayUnits = [...unitAllocations].sort(
        (a, b) => b.weightScore - a.weightScore,
      );

      dayUnits.forEach((unit) => {
        const remainingHours =
          unit.allocatedHours -
          dayHoursUsed * (1 / availability.weeklyDays.length);
        ...
        if (remainingHours > 0 && dayHoursUsed < availability.dailyHours) {
          ...
          const blockHours = Math.min(
            blockDuration / 60,
            remainingHours,
            availability.dailyHours - dayHoursUsed,
          );
      ...
      if (availability.studyPreference !== "short" && dayHoursUsed > 2) {
        ...
        blocks.push({
          ...
          endTime: calculateEndTime(
            calculateStartTime(breakTime, availability.preferredTimes),
            0.25,
          ),
          duration: 15,
          type: "break",
    ...
    const averageBlockDuration =
      blocks.reduce((sum, block) => sum + block.duration, 0) / blocks.length;
    const hardUnitsRatio =
      units.filter((u) => u.difficulty === "hard").length / units.length;
    stressIndex = Math.min(
      100,
      (averageBlockDuration / 90) * 50 + hardUnitsRatio * 50,
    );
  ...
  const calculateStartTime = (
    ...
    const baseHour = preferredTimes.includes("morning")
      ? 8
        ...
        {units.length === 0 ? (
          ...
          <VStack spacing={4}>
            ...
            <Button
              ...
              disabled={
                units.length === 0 || availability.weeklyDays.length === 0
              }
            >
              Generate Study Plan
            </Button>
