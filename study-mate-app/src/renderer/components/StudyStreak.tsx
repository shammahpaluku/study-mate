...
const StudyStreak: React.FC = () => {
  ...
  const generateStreakHistory = (activities: any[]): { date: string; hasActivity: boolean }[] => {
  ...
  const calculateMonthlyStreaks = (activities: any[]): { month: string; streak: number }[] => {
    ...
    activities.forEach((activity: any) => {
      ...
      const monthActivities = activities.filter((act: any) => {
        ...
        return actDate.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) === monthKey;
      });
  ...
  return (
    <Box p={6} borderWidth={1} borderRadius="lg" borderColor={borderColor} bg={cardBg} boxShadow="sm">
      <VStack spacing={6} align="stretch">
        ...
        <Box p={4} borderRadius="lg" bg={`${getStreakColor(streakData.currentStreak)}.50`} borderWidth={1} borderColor={`${getStreakColor(streakData.currentStreak)}.200`}>
          <VStack spacing={3} align="center">
            <HStack spacing={2}>
              <Icon as={FiAward} boxSize={6} color={`${getStreakColor(streakData.currentStreak)}.500`} />
              <Text fontSize="3xl" fontWeight="bold" color={`${getStreakColor(streakData.currentStreak)}.700`}>
                {streakData.currentStreak}
              </Text>
            </HStack>
            <Text fontSize="lg" fontWeight="600" color={`${getStreakColor(streakData.currentStreak)}.600`}>
              Day Streak
            </Text>
            <Text fontSize="sm" color={`${getStreakColor(streakData.currentStreak)}.600`}>
