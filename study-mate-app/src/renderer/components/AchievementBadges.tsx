...
import { Box, VStack, HStack, Text, Heading, Badge, Icon, useColorModeValue } from '@chakra-ui/react';
import { FiAward, FiStar, FiTrendingUp, FiTarget, FiClock } from 'react-icons/fi';
...
const AchievementBadges: React.FC = () => {
  ...
  const checkAchievements = () => {
    ...
    localStorage.setItem('studyAchievements', JSON.stringify(updatedAchievements));
  };
          ...
          {achievements.map(achievement => {
            ...
            return (
              <Box
                ...
                borderWidth={1}
                borderRadius="md"
                borderColor={achievement.unlocked ? `${categoryColor}.200` : borderColor}
              ...
              >
                <HStack spacing={3} align="start">
                  <Icon
                    as={IconComponent}
                    boxSize={6}
                    color={achievement.unlocked ? `${categoryColor}.500` : 'gray.400'}
                  />
                  <VStack align="start" spacing={1} flex={1}>
                    <HStack spacing={2}>
                      <Text fontWeight="600" color={achievement.unlocked ? 'gray.900' : 'gray.600'}>
                        {achievement.title}
                      </Text>
